...
config.base_ui = function()
  ...
  local function override_ui_input()
    ...
    function UIInput:init(opts, on_done)
      ...
      UIInput.super.init(self, {
        ...
        border = {
          highlight = "FloatBorder:DeusInputBorder",
          style = { " ", " ", " ", " ", " ", " ", " ", " " },
          text = {
            top = Text(border_top_text, "DeusInputBorder"),
            top_align = "center",
          },
        },
        win_options = {
          winhighlight = "Normal:DeusInputNormal",
        },
      }, {
        prompt = require("configs.base.ui.icons").common.separator .. " ",
  ...
  local function override_ui_select()
    ...
    function UISelect:init(items, opts, on_done)
      ...
      local kind = opts.kind or "unklatern"
      local format_item = opts.format_item or function(item) return tostring(item.__raw_item or item) end
      local popup_options = {
        relative = "editor",
        position = "50%",
        border = {
          highlight = "FloatBorder:DeusSelectBorder",
          style = { " ", " ", " ", " ", " ", " ", " ", " " },
          text = {
            top = Text(border_top_text, "DeusSelectBorder"),
            top_align = "center",
          },
        },
        win_options = {
          winhighlight = "Normal:DeusSelectNormal",
        },
        zindex = 999,
      }
  ...
  return {
    override_ui_input = override_ui_input,
