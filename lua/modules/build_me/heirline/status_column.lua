...
function M.get_statuscolumn()
  ...
  local function get_signs(_, lnum)
    local signs = {}
  ...
  return {
    {
      condition = function()
        return not conditions.buffer_matches({
          buftype = require("modules.build_me.heirline.buftypes"),
          filetype = require("modules.build_me.heirline.filetypes"),
        })
      end,
      static = {
        ...
        click_args = function(self, minwid, clicks, button, mods)
          ...
          if sign == " " then sign = fn.screenstring(args.mousepos.screenrow, args.mousepos.screencol - 1) end
        ...
        resolve = function(self, name)
          for pattern, callback in pairs(self.handlers.Signs) do
            if name:match(pattern) then return vim.defer_fn(callback, 100) end
          end
        end,
        handlers = {
          Signs = {
            ["Neotest.*"] = function(self, args) require("neotest").run.run() end,
            ["Debug.*"] = function(self, args) require("dap").continue() end,
            ["Diagnostic.*"] = function(self, args) vim.diagnostic.open_float() end,
            ["LspLightBulb"] = function(self, args) vim.lsp.buf.code_action() end,
          },
          Dap = function(self, args) require("dap").toggle_breakpoint() end,
          Fold = function(args)
            local line = args.mousepos.line
            if fn.foldlevel(line) <= fn.foldlevel(line - 1) then return end
            vim.cmd.execute("'" .. line .. "fold" .. (fn.foldclosed(line) == -1 and "close" or "open") .. "'")
          end,
          GitSigns = function(self, args)
            vim.defer_fn(function() require("gitsigns").blame_line({ full = true }) end, 100)
          end,
        },
      },
      init = function(self) self.signs = {} end,
      spacer,
      ...
      -- Folds
      {
        init = function(self) self.can_fold = fn.foldlevel(v.lnum) > fn.foldlevel(v.lnum - 1) end,
        {
          condition = function(self) return v.virtnum == 0 and self.can_fold end,
          provider = "%C",
        },
        {
        ...
        on_click = {
          name = "sc_fold_click",
          callback = function(self, ...) self.handlers.Fold(self.click_args(self, ...)) end,
        },
      },
      -- Git Signs
      {
        {
          ...
          {
            provider = "│",
            hl = function(self) return self.sign or { fg = colors.fg.hex } end,
            on_click = {
              name = "sc_gitsigns_click",
              callback = function(self, ...) self.handlers.GitSigns(self.click_args(self, ...)) end,
            },
          },
        },
        {
