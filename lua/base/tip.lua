--Start-of-file--
-- TIP COMPONENT --
-- This component is a modified copy of the Alpha fortune module.
...
local format_tip = function(tip, max_width)
  -- Converts list of strings to one formatted string (with linebreaks)
  local formatted_tip = { ' ' } -- adds spacing between alpha-menu and footer
  for _, line in ipairs(tip) do
    local formatted_line = format_line(line, max_width)
    formatted_tip = list_extend(formatted_tip, formatted_line)
  end
  return formatted_tip
end

local get_tip = function(tip_list)
  -- selects an entry from fortune_list randomly
  math.randomseed(os.time())
  local ind = math.random(1, #tip_list)
  return tip_list[ind]
end
...
local main = function(opts)
  ...
  local tip = get_tip(options.fortune_list)
  local formatted_tip = format_tip(tip, options.max_width)
