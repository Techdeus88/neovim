...
--  DESCRIPTION: General utility functions to use within Nvim.
...
--      -> is_list_empty            -> Check the preseence of a element inside of a lua table.
...
function U.run_cmd(cmd, show_error)
...
function U.add_autocmds_to_buffer(augroup, bufnr, autocmds)
...
function U.del_autocmds_from_buffer(augroup, bufnr)
  -- Attempt to retrieve existing autocmds associated with the specified augroup and bufnr
  local cmds_found, cmds = pcall(vim.api.nvim_get_autocmds, { group = augroup, buffer = bufnr })
...
--- Same as using funcs.notify, but it saves us typing the title every time.
--- @param msg string The notification body.
--- @param type number|nil The type of the notification (:help vim.log.levels).
--- @param opts? table The nvim-notify options to use (:help notify-options).
function U.notify(msg, type, opts)
  vim.schedule(function()
    funcs.notify(msg, type, vim.tbl_deep_extend('force', { title = 'Neovim' }, opts or {}))
  end)
end
...
function U.get_plugin_opts(plugin)
...
function U.open_with_program(path)
  ...
  if not cmd then
    U.notify('Available system opening tool not found!', vim.log.levels.ERROR)
  end
...
function U.get_mappings_template()
  local maps = {}
    ...
    for keymap, options in pairs(maps) do
      ...
      if options then
        ...
        if not cmd then -- if which-key mapping, queue it
          ...
          if not U.which_key_queue then
            U.which_key_queue = {}
          end
          table.insert(U.which_key_queue, keymap_opts)
...
function U.trigger_event(event, is_urgent)
  -- define behavior
...
--- @return string icon.
function U.get_icon(icon_name, fallback_to_empty_string)
  -- guard clause
  ...
  if not U[icon_pack] then -- only if not cached already.
    if icon_pack == 'icons' then
      U.icons = require('base.ui.icons')
    elseif icon_pack == 'fallback_icons' then
      U.fallback_icons = require('base.ui.fallback_icons')
    end
  end

  -- return specified icon
  local icon = U[icon_pack] and U[icon_pack][icon_name]
  return icon
end

function U.is_available(plugin_name)
  -- Check if the plugin is installed via MiniDeps
  local deps_location = plugin_name == 'mini.nvim' and 'start' or 'opt'
  local mini_deps_path = vim.fn.stdpath('data') .. '/site/pack/deps/' .. deps_location .. '/' .. plugin_name
  if vim.fn.isdirectory(mini_deps_path) == 1 then
    return true
  end
  return false
end

function U.is_list_empty(tbl)
  local is_empty = next(tbl) == nil
  return is_empty
end

local M = {}

function U.is_active()
  local winid = vim.api.nvim_get_current_win()
  local curwin = tonumber(vim.g.actual_curwin)
  return winid == curwin
end

function U.is_not_active()
...
function U.buffer_matches(patterns, bufnr)
  bufnr = bufnr or 0
...
function U.width_percent_below(n, thresh, is_winbar)
  local winwidth
...
function U.get_icons(type)
  type = type or vim.g.fallback_icons_enabled and "fallback" or "base"
  local icon_map = {
    base = "base.ui.icons",
    fallback = "base.ui.fallback_icons",
    heirline = "base.ui.heirline_icons",
  }
  local icon_list = icon_map[type]
  local ok, icons = pcall(require, icon_list)
  if not ok then return {} end
  return icons
end

function U.is_git_repo()
  return vim.b.gitsigns_head or vim.b.gitsigns_status_dict
end

function U.has_diagnostics()
  return #vim.diagnostic.get(0) > 0
end

function U.lsp_attached()
  return next(vim.lsp.get_clients({ bufnr = 0 })) ~= nil
end

-- Filter function that takes a list and a predicate function
...
function U.toggle_boo_statusline(show_type)
  local funcs = require("core.funcs")
  local show_types = { "toggle", "window", "file_info" }
  local settings = Global.settings

  if show_type ~= nil then
    assert(vim.tbl_contains(show_types, show_type),
      string.format('Show type must be one of  the show types: %s', vim.inspect(show_types)))
    settings.statusline['boo_incline'] = show_type
